...
Dialogue_Marquee :: struct { t: f32 }
Dialogue_Pause :: struct { t: f32 }
	...
	case Dialogue_Marquee:
		...
		if s.t <= 0 {
			if done := advance_marquee(); done {
				...
				dialogue_state = Dialogue_Marquee{ t = dialogue_speed }
			}
		}
	case Dialogue_Pause:
		s.t -= dt
...
set_next_dialogue_state :: proc() {
	if dialogue_pause > 0 {
		dialogue_state = Dialogue_Pause{t = dialogue_pause}
		dialogue_pause = 0
