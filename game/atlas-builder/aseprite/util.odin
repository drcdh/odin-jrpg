...
get_property_type :: proc(v: Property_Value) -> (type: WORD, err: Marshal_Error) {
	switch t in v {
	case nil:
		type = WORD(Property_Type.Null)
	case bool:
		type = WORD(Property_Type.Bool)
	case i8:
		type = WORD(Property_Type.I8)
	case BYTE:
		type = WORD(Property_Type.U8)
	case SHORT:
		type = WORD(Property_Type.I16)
	case WORD:
		type = WORD(Property_Type.U16)
	case LONG:
		type = WORD(Property_Type.I32)
	case DWORD:
		type = WORD(Property_Type.U32)
	case LONG64:
		type = WORD(Property_Type.I64)
	case QWORD:
		type = WORD(Property_Type.U64)
	case FIXED:
		type = WORD(Property_Type.Fixed)
	case FLOAT:
		type = WORD(Property_Type.F32)
	case DOUBLE:
		type = WORD(Property_Type.F64)
	case STRING:
		type = WORD(Property_Type.String)
	case POINT:
		type = WORD(Property_Type.Point)
	case SIZE:
		type = WORD(Property_Type.Size)
	case RECT:
		type = WORD(Property_Type.Rect)
	case UUID:
		type = WORD(Property_Type.UUID)
	case UD_Vec:
		type = WORD(Property_Type.Vector)
	case Properties:
		type = WORD(Property_Type.Properties)
	case:
		err = Marshal_Errors.Invalid_Property_Type
	}
	return
}
...
@(private)
fast_log_str_enum :: proc(
	lvl: log.Level,
	str: string,
	val: $T,
	sep := " ",
	loc := #caller_location,
) where ir.type_is_enum(T) {
	logger := context.logger
	if logger.procedure == nil {return}
	if lvl < logger.lowest_level {return}
...
@(private)
fast_log_str_num :: proc(
	lvl: log.Level,
	str: string,
	val: $T,
	sep := " ",
	loc := #caller_location,
) where ir.type_is_numeric(T) {
	logger := context.logger
	if logger.procedure == nil {return}
	if lvl < logger.lowest_level {return}
