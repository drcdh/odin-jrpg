...
start_level_cave :: proc() {
	...
	add_world_entity(Entity{id = 5, tile = LEVEL_CAVE_FIRE_0, v = animation_create(.Fire), talk = torch, light = 3})

	add_world_entity(Entity{id = 6, tile = LEVEL_CAVE_FIRE_1, v = animation_create(.Fire), talk = torch, light = 3})

	add_world_entity(Entity{id = 7, tile = LEVEL_CAVE_FIRE_2, v = animation_create(.Fire), talk = torch, light = 3})

	add_world_entity(Entity{id = 8, tile = LEVEL_CAVE_FIRE_3, v = animation_create(.Fire), talk = torch, light = 3})
