...
draw :: proc() {
	...
	rl.DrawTextEx(game.font, fmt.caprintf("% 3d: %s", bid, bt.name), {0, 0}, 32, 0, rl.WHITE)
	for i in 0 ..< game.NUM_STATS {
		rl.DrawTextEx(
			game.font,
			strings.clone_to_cstring(game.stat_string(bt, game.Stat(i)), context.temp_allocator),
			{0, f32(48 + i * 48)},
			32,
			0,
			rl.WHITE,
